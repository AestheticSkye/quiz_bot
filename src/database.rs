...
impl Database {
	...
	pub async fn save_quiz_creation(&self, owner_id: u64, text: &str) -> QuizCreation {
		...
		quiz_creation
			...
			.await
			.expect("Failed to insert quiz_creation")
	}

	pub async fn fetch_quiz(&self, quiz_id: &Uuid) -> Option<Quiz> {
		prelude::Quiz::find_by_id(*quiz_id)
			.one(&self.connection)
			.await
			.unwrap()
	}

	pub async fn fetch_questions(&self, quiz: &Quiz) -> Vec<Question> {
		prelude::Question::find()
	...
	pub async fn fetch_answers(&self, question: &Question) -> Vec<Answer> {
		prelude::Answer::find()
	...
	pub async fn fetch_quiz_creation(&self, owner_id: u64) -> Option<QuizCreation> {
		prelude::QuizCreation::find()
	...
	pub async fn fetch_current_creation_question(
		...
		prelude::Question::find_by_id(current_question_id)
			.one(&self.connection)
			.await
			.unwrap()
	}

	pub async fn add_question(&self, quiz_creation: QuizCreation, question: &str) -> QuizCreation {
		...
		let question = question.insert(&self.connection).await.unwrap();

		let mut quiz_creation: quiz_creation::ActiveModel = quiz_creation.into();

		quiz_creation.current_question_id = Set(Some(question.id));

		quiz_creation.update(&self.connection).await.unwrap()
	}
...
async fn test() {
	...
	let quiz_creation = db.fetch_quiz_creation(42).await.unwrap();
